...
def seed_transactions(products, brands, vendors, customers, users):
    ...
    for _ in range(num_purchases):
        ...
        ptype = random.choices(['Wholesale', 'Retail', 'Both'], weights=[0.45, 0.45, 0.10])[0]
        cust = random.choice(customers)
        customer_id = cust.id if ptype in ['Retail', 'Both'] and random.random() < 0.8 else None
        c_name = cust.name if cust else 'General Customer'
        c_phone = cust.phone if cust else '9876543210'
        ...
        purchase = Purchase(
            ...
            brand_name=product.brand.name if product.brand else 'Generic Brand',
            model_name=product.model or product.name,
            quantity=qty,
            ...
            email=c_email,
            product_type=product.category.name if product.category else ptype,
            sale_type=ptype,
            invoice='sample_invoice.pdf' if random.random() < 0.7 else '',
            attachment='sample_spec.pdf' if random.random() < 0.5 else '',
        )
