...
class SQLiteStorage:
    ...
    def _create_schema(self):
        ...
        with self._connect() as conn:
            ...
            new_cols = [
                ...
                ('customer_name', 'TEXT'), ('phone_number', 'TEXT'), ('email', 'TEXT'),
                ('product_type', 'TEXT'), ('sale_type', 'TEXT'), ('invoice', 'TEXT'),
                ('updated_at', 'TEXT')
            ]
            ...
            # Migrate whats_app_message table: add phone_number and message columns if missing
            wa_cols = [r['name'] for r in conn.execute('PRAGMA table_info(whats_app_message)').fetchall()]
            for col_name, col_type in [('phone_number', 'TEXT'), ('message', 'TEXT')]:
                if col_name not in wa_cols:
                    try:
                        conn.execute(f'ALTER TABLE whats_app_message ADD COLUMN {col_name} {col_type}')
                    except Exception:
                        pass
    ...
    def search_purchases(self, query):
        q=(query or '').lower()
        return [x for x in self.purchases if q in (x.purchase_id or '').lower()
                or q in (x.brand_name or '').lower()
                or q in (x.model_name or '').lower()
                or q in (x.customer_name or '').lower()
                or q in (x.phone_number or '').lower()
                or q in (x.product_type or '').lower()
                or q in (x.sale_type or '').lower()]
    def delete_purchase(self, item_id):
        self.purchases = [x for x in self.purchases if x.id != item_id]
        self._save_collection('purchases')
        return True
    ...
    def delete_brand(self, brand_id):
        self.brands = [b for b in self.brands if b.id != brand_id]
        self._save_collection('brands')
        return True
    def delete_category(self, category_id):
        self.categories = [c for c in self.categories if c.id != category_id]
        self._save_collection('categories')
        return True
